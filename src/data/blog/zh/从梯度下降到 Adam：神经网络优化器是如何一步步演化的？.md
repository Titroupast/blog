...
但普通梯度下降会出现很多问题：走得太慢、来回震荡、学习率不好选、遇到平坦区域速度很慢、不同参数最好使用不同学习率、在复杂曲面上表现很差。于是出现了两条改进路线：SGD → Momentum → Nesterov，以及 AdaGrad → RMSProp，最终两条路线汇合成 Adam（$Adam\approx Momentum+RMSProp$）。这两条路线各管一件事：**SGD → Momentum → Nesterov 优化的是「更新方向」（往哪走），AdaGrad → RMSProp 优化的是「更新步长」（走多远）**。这篇文章就按照这个发展过程，一步一步理解这些优化算法究竟解决了什么问题。
